...
function UserFan(props) {
    ...
    const [totalFanNftsData, setTotalFanNftsData] = useState(0);
    ...
    let getUserData = (start, end) => {
        ...
        axios.get(`/nft/getprofileusernft/${props.userId}/Fan/${start}/${end}`).then(
            (response) => {
                console.log("response", response);
                setData(response.data.NFTdata);
                setTotalFanNftsData(response.data.NFTcount);
                handleCloseBackdrop();
            },
            (error) => {
